	...
	switch(d)
	{
		case '1':
...

			{
				...
				{
				while(fread(&p,sizeof(reddit),1,fp))
				s=1+p.sn;
				}
				fclose(fp);
			}
		...
		case '4':
			{
				...
				while(fread(&p,sizeof(reddit),1,fp))
					if(p.sn!=y)
					fwrite(&p,sizeof(reddit),1,f1);
				clrscr();
				mainscreen();
				fseek(fp,(y-1)*sizeof(reddit),SEEK_SET);
				fread(&p,sizeof(reddit),1,fp);
				print();
				edit1();
				gotoxy(25,46);
				...
				case '1':
					{
					 ...
					 fseek(fp,0,SEEK_END);
					 tsz=ftell(fp);
					 n=(int)(tsz/sizeof(reddit));
					 for(i=0;i<(n-1);i++)
					 {
						for(a=i+1;a<n;a++)
						{
						fseek(fp,i*sizeof(reddit),SEEK_SET);
						fread(&p,sizeof(reddit),1,fp);
						fseek(fp,a*sizeof(reddit),SEEK_SET);
						fread(&q,sizeof(reddit),1,fp);
						if(strcmp(p.name,q.name)>0)
							{
							fseek(fp,i*sizeof(reddit),SEEK_SET);
							fwrite(&q,sizeof(reddit),1,fp);
							fseek(fp,a*sizeof(reddit),SEEK_SET); fflush(stdin);
							fwrite(&p,sizeof(reddit),1,fp);
							}
						}
					}
					rewind(fp);
...
void mainscreen()
{
	...
	cprintf("THE BESTEST HOSPITAL");
	gotoxy(27,6);textcolor(6);
	cprintf("NYC, New York");
